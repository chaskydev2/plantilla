...
class JobContractorController extends Controller
{
    /**
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $job = JobContractor::create($validated);
    ...
    {
        $job = JobContractor::with(['creator', 'homeowner'])->findOrFail($id);
    ...
    {
        $job = JobContractor::findOrFail($id);
        ...
        DB::beginTransaction();
        try {
            // Manejo de imagen: subir nueva o borrar existente
    ...
    {
        $stats = [
            'total' => JobContractor::count(),
            'active' => JobContractor::active()->count(),
            'inactive' => JobContractor::inactive()->count(),
            'total_amount_paid' => JobContractor::sum('amount_paid'),
        ];
