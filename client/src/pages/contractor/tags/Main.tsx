...
// Servicio adaptado para useResource con contractorId
const TagServiceByContractor = {
	getAllPaginated: ItemService.getByContractor,
};

// Obtener el ID del contratista desde localStorage
...
export default function TagList() {
	...
	const {
		...
		service: contractorId ? TagServiceByContractor : ItemService,
		resourceId: contractorId ?? undefined,
	...
	const handleDelete = async (item: IItemResource) => {
		...
		try {
			let response;
			if (contractorId) {
				response = await ItemService.removeForContractor({
					contractor_user_id: contractorId,
					tag_id: Number(item.id),
				});
			} else {
				response = await ItemService.remove(item.id);
			}
