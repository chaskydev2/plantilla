...
const columns = [
  ...
  {
    key: "job_post_id",
    header: "ID de Publicación",
    render: (item: IItemResource) => (
      <div>{item.job_post_id}</div>
    ),
    sortable: true,
  },
  {
    key: "contractor_id",
    header: "ID de Contratista",
    render: (item: IItemResource) => (
      <div>{item.contractor_id}</div>
    ),
    sortable: true,
  },
  {
...
export default function JobApplicationList() {
  ...
  const confirmDelete = (item: IItemResource) => {
    openDialog(
      ...
      `¿Estás seguro que deseas eliminar la postulación #${item.id}?`,
      () => handleDelete(item),
      "danger"
    );
  };
  ...
  const handleDelete = async (item: IItemResource) => {
    try {
      ...
      toastify.success(response?.message || "Postulación eliminada");
      fetchItems();
    } catch (error) {
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle={t('jobApplications.pageTitle', 'Postulaciones')} />
      <DataTable
      ...
      />
      {/* El formulario de edición/agregado ha sido removido. Solo se muestra la tabla y el diálogo de confirmación. */}
      {dialogConfig && (
        <ConfirmDialog
