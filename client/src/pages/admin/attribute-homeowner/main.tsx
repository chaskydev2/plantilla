...
import AttributeHomeOwnerModal from './AttributeHomeOwnerModal';
import AttributeHomeownerTable from './AttributeHomeownerTable';
...
const AttributeHomeowners: React.FC = () => {
  ...
  const confirmDelete = (item: IItemResource) => {
    openDialog(
      ...
      `¿Estás seguro que deseas eliminar el atributo del propietario #${item.id}?`,
      () => handleDelete(item),
      'danger'
    );
  };
  ...
  const columns: import('@/core/types/ITable').ITableColumn<WithRelations>[] = [
    {
      ...
      render: (item: WithRelations) =>
        h(
          ...
          h('div', { className: 'font-bold text-gray-700 dark:text-gray-300' }, String(item.id))
        ),
      sortable: true,
    },
    {
    ...
    h(
      ...
      h(
        ...
        h(
          ...
          {
            className:
              'absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-gray-700 dark:text-gray-300',
          },
  ...
  return h(
    'div',
    null,
    h(PageBreadcrumb, { pageTitle: 'Atributos de Propietarios' }),
    h(AttributeHomeownerTable, {
...
export default AttributeHomeowners;
